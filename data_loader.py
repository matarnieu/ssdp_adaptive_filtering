...
def generate_synthetic_data(num_samples, low, high, size_filter, snr=None, type_signal='sinus', type_filter='dirac'):
    #generate signal
    ...
    H = generate_time_varying_filter(type=type_filter, size_filter=size_filter, timestep=num_samples) 
...
def main(plot=False):
    ...
    noisy_signal, signal, noise, h = generate_synthetic_data(num_samples=num_samples, low=low, high=high, size_filter=size_filter, snr=snr, type_signal=type_signal, type_filter=type_filter)
    
    if plot:
        plot_2(noisy_signal, signal, h, type_signal, type_filter, snr, window_start=1000, window_size=200)
